...
def search_free_games():
    ...
    # Dig through the dictionary of game information
...
class claim_free_games(BaseCase):
    ...
    def test_initiator(self):
        ...
        free_game_checklist = free_games.copy()

        if len(free_games) < 1:
            self.driver.quit()
            pytest.exit("No current free games on Epic Games.")

        print("Found free games. Logging into EGS.")
        self.login_to_egs()

        # Check if the new game found isn't already in the library or cart.
        try:
            with open(game_log_directory, "r+") as game_log_file:
                game_id_log = []
                try:
                    game_id_log = json.load(game_log_file)
                except json.JSONDecodeError:
                    print("Game Log Content is either empty or is not valid.")

                for game_id, url in free_games.items():
                    if game_id not in game_id_log:
                        self.add_game_to_cart(url)
                        game_id_log.append(game_id)
                    else:
                        print(f"{game_id} has already been found in games_log.txt.")
                        del free_game_checklist[game_id]

                game_log_file.seek(0)
                json.dump(game_id_log, game_log_file, indent=4)
        except FileNotFoundError:
            print(f"File not found: {game_log_directory}")

        if len(free_game_checklist) < 1:
            self.driver.quit()
            pytest.exit("Found no new free games")
